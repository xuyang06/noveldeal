...
def form_urls():
	...
	pre_str_1 = 'http://www.bergdorfgoodman.com/Sale/Shop-All-Sale/cat441307_cat205700_cat000000/c.cat#userConstrainedResults=true&refinements=&page='
	after_str_1 = '&pageSize=30&sort=PCS_SORT&definitionPath=/nm/commerce/pagedef_rwd/template/EndecaDriven&onlineOnly=&allStoresInput=false&rwd=true&catalogId=cat441307'
	page_num = 55
...
class BergdorfSpider(scrapy.Spider):
	...
	def parse1(self, response):
		...
		hrefs = sel.xpath('//p[contains(@class, "price-adornment priceadorn price-adornment-highlight")]/../../div[contains(@class, "productdesigner OneLinkNoTx")]/a/@href').extract()
		#images = sel.xpath('//img[contains(@class, "product-thumbnail-image")]/@src').extract()
		brands = sel.xpath('//p[contains(@class, "price-adornment priceadorn price-adornment-highlight")]/../../div[contains(@class, "productdesigner OneLinkNoTx")]/a/text()').extract()
		products = sel.xpath('//p[contains(@class, "price-adornment priceadorn price-adornment-highlight")]/../../div[contains(@class, "productname hasdesigner OneLinkNoTx")]/a/text()').extract()
		pre_prices = sel.xpath('//p[contains(@class, "price-adornment priceadorn strikethrough")]/span[contains(@class, "price OneLinkNoTx")]/text()').extract()
		sale_prices = sel.xpath('//p[contains(@class, "price-adornment priceadorn price-adornment-highlight")]/span[contains(@class, "price OneLinkNoTx")]/text()').extract()
		...
		log.debug('the page has total %s pre_prices ...', str(len(pre_prices)))
		log.debug('the page has total %s sale_prices ...', str(len(sale_prices)))
		for i in range(len(hrefs)):
			item = ProductItem()
			item['time'] = str(self.dt.year) + '-' + str(self.dt.month) + '-' + str(self.dt.day) + '-' + str(self.dt.hour)
			item['site'] = 'bergdorfgoodman.com'
			item['brand'] = brands[i].strip()
			#item['image'] = images[i].strip()
			item['image'] = ''
			#href = 'http://shop.nordstrom.com' + hrefs[i].strip()
			item['href'] = 'http://www.bergdorfgoodman.com' + hrefs[i].strip()
