...
def form_urls():
	...
	pre_str_1 = 'http://www.bluefly.com/designer-womens?vl=l&ppp=48&cp='
	after_str_1 = '&clearancePrice_OR_reducedPrice=0~*_OR_0~*'
	page_num = 120
	for num in range(1, page_num + 1):
		url1 = form_url(num, pre_str_1, after_str_1)
		urls.append(url1)
	pre_str_1 = 'http://www.bluefly.com/designer-shoes?vl=l&ppp=48&cp='
	after_str_1 = '&clearancePrice_OR_reducedPrice=0~*_OR_0~*'
	page_num = 26
	for num in range(1, page_num + 1):
		url1 = form_url(num, pre_str_1, after_str_1)
		urls.append(url1)
	pre_str_1 = 'http://www.bluefly.com/designer-handbags-accessories?vl=l&ppp=48&cp=*'
	after_str_1 = '&clearancePrice_OR_reducedPrice=0~*_OR_0~*'
	page_num = 90
	for num in range(1, page_num + 1):
		url1 = form_url(num, pre_str_1, after_str_1)
		urls.append(url1)
	pre_str_1 = 'http://www.bluefly.com/designer-mens?vl=l&ppp=48&cp='
	after_str_1 = '&clearancePrice_OR_reducedPrice=0~*_OR_0~*'
	page_num = 185
	for num in range(1, page_num + 1):
		url1 = form_url(num, pre_str_1, after_str_1)
...
class BlueflySpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		hrefs = sel.xpath('//span[contains(@class, "productPriceSalePrice")]/../div[@class="listBrand"]/a/@href').extract()
		#images = sel.xpath('//span[contains(@class, "productPriceSalePrice")]/../../div/a/img[contains(@class, "imageBorder")]/@src').extract()
		brands = sel.xpath('//span[contains(@class, "productPriceSalePrice")]/../div[@class="listBrand"]/a/text()').extract()
		products = sel.xpath('//span[contains(@class, "productPriceSalePrice")]/../div/div[@class="productShortName"]/a/text()').extract()
		pre_prices = sel.xpath('//span[contains(@class, "productPriceSalePrice")]/../span[contains(@class, "productPriceRetail")]/text()').extract()
		sale_prices = sel.xpath('//span[contains(@class, "productPriceSalePrice")]/text()').extract()
		...
		#log.debug('the page has total %s sale_prices1 ...', str(len(sale_prices1)))
		#log.debug('the page has total %s sale_prices2 ...', str(len(sale_prices2)))
		for i in range(len(hrefs)):
			item = ProductItem()
			item['time'] = str(self.dt.year) + '-' + str(self.dt.month) + '-' + str(self.dt.day) + '-' + str(self.dt.hour)
			item['site'] = 'bluefly.com'
			item['brand'] = brands[i].strip()
			#item['image'] = images[i].strip()
			item['image'] = ''
			#href = 'http://shop.nordstrom.com' + hrefs[i].strip()
			item['href'] = 'http://www.bluefly.com' + hrefs[i].strip()
