...
def form_urls():
	...
	#page_num = 5;
	urls = []
	pre_str_1 = 'http://www1.bloomingdales.com/shop/sale/all-sale-clearance/Pageindex/'
	after_str_1 = '?id=1003304'
	...
	#http://www1.bloomingdales.com/shop/sale/all-sale-clearance/Pageindex/2?id=1003304

class BloomSpider(scrapy.Spider):
	name = "bloomingdales"
	allowed_domains = ["www1.bloomingdales.com"]
	#start_urls = [
	...
	def parse(self, response):
		...
		hrefs = sel.xpath('//div[@class="shortDescription"]/a/@href').extract()
		images = sel.xpath('//img[@class="thumbnailImage crossfadeImage"]/@src').extract()
		brands = sel.xpath('//div[@class="shortDescription"]/a/text()').extract()
		#products = sel.xpath('//p[@class="product-description"]/text()').extract()
		pre_prices = sel.xpath('//span[@class="priceBig"]/text()').extract()
		sale_prices = sel.xpath('//span[@class="priceSale"]/text()').extract()
		...
		for i in range(len(hrefs)):
			...
			item['site'] = 'bloomingdales.com'
			item['brand'] = brands[i].strip()
			item['image'] = images[i].strip()
			#item['image'] = ''
			href = hrefs[i].strip()
			item['href'] = href[0:href.index('#fn%')]
