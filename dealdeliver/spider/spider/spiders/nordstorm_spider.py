...
class NordstormSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		hrefs = sel.xpath('//a[@class="title"]/@href').extract()
		#images = sel.xpath('//img[@class="thumbnailImage crossfadeImage"]/@src').extract()
		brands = sel.xpath('//a[@class="title"]/text()').extract()
		#products = sel.xpath('//p[@class="product-description"]/text()').extract()
		pre_prices = sel.xpath('//span[@class="price regular"]/text()').extract()
		sale_prices = sel.xpath('//span[@class="price sale"]/text()').extract()
